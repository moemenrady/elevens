...
@section('content')
            ...
            <div class="section">
                ...
                <div class="box">
                    ...
                    <div class="booking-time">
                        ...
                        <div class="time-item">
                            ...
                            <span class="value">{{ \Carbon\Carbon::parse($booking->end_at)->format('Y-m-d h:i A') }}</span>
                        </div>
                        ...
                        @if (!empty($actual_duration))
                            @php
                                ...
                                if ($totalMin < 30) {
                                    $display = 'لا يوجد ساعات';
                                } else {
                                    // حساب عدد نصف الساعات كاملة
                                    $halfHours = intdiv($totalMin, 30);

                                    $hours = intdiv($halfHours, 2); // عدد الساعات الكاملة
                                    $half = $halfHours % 2; // نصف ساعة
                            ...
                            @endphp
                            <div class="time-item duration" id="durationRow" style="cursor:pointer;">
                                <span class="label">⏱️ المدة:</span>
                                <span class="value" id="durationValue">{{ $display }}</span>
                    ...
                    @if ($booking->status === 'in_progress')
                        ...
                        <div class="inline-edit-row" style="margin-top:8px;">
                            <form id="inlineEditForm"
                                ...
                                @csrf
                                @method('PUT')
                                <input id="start_time_inline" name="real_start_time" type="datetime-local"
                                    value="{{ \Carbon\Carbon::parse($booking->real_start_at)->format('Y-m-d\TH:i') }}"
                        ...
                        <script>
                            document.addEventListener("DOMContentLoaded", function() {
                                ...
                                inlineForm.addEventListener('submit', async e => {
                                    ...
                                    const url = "{{ route('bookings.updateRealStartTime', $booking->id) }}";
                        ...
                        <div class="products-list">
                            ...
                            @foreach ($importantProducts as $importantProduct)
                                <form class="invoiceForm" action="{{ route('booking.purchase.store', $booking->id) }}"
                                    ...
                                    <button type="submit"
                                        class="product-item"data-id="{{ $importantProduct->product_id }}">
                ...
                <div class="box">
                    ...
                    <p><strong>الإجمالي الفعلي حتى الآن:</strong>
                        <span id="combinedTotal" class="price" style="color:red; font-weight:800;">
  ...
  ">
                    ...
                    @elseif($booking->status === 'in_progress')
                        ...
                        <form id="checkoutForm" action="{{ route('booking.checkout', $booking->id) }}" method="POST"
                            ...
                            <div style="margin-bottom:10px;">
                                <button type="button" id="toggleMinutesMode" class="btn btn-dark">
                                    ⏱️ عرض الحساب بالدقائق: <span id="modeText">OFF</span>
                                </button>
                            </div>
        ...
        <script>
            document.addEventListener("DOMContentLoaded", () => {
                ...
                if (checkoutForm) {
                    checkoutForm.addEventListener('submit', function(e) {
                        e.preventDefault(); // وقف الإرسال لأول مرة
                        const hasRemainingTime = {{ $hasRemainingTime ? 'true' : 'false' }};

                        if (!isMinutesMode) {
                            if (hasRemainingTime) {
                                alert('❌ لازم تضيف كل الفترات (تقسيم الوقت) قبل إنهاء الحساب');
                                return;
                            }

                            const slots = @json($booking->timeSlots);
                            const invalidSlot = slots.find(s => !s.attendees_count || s.attendees_count <= 0);

                            if (invalidSlot) {
                                alert('❌ لازم تضيف عدد الأفراد لكل فترة');
                                return;
                            }
                        }
                        // **1. جمع المشتريات**
            ...
            // **4. وظيفة اختيار الدفع**
            function choosePayment(type) {
                document.getElementById('payment_type').value = type;

                // ابعت الفورم بعد الاختيار
        ...
        <script>
            ...
            const bookingKey = 'minutesMode_{{ $booking->id }}'; // 🔥 مفتاح خاص بكل حجز

            const toggleBtn = document.getElementById('toggleMinutesMode');
            const hoursSpan = document.querySelector('.price-hours');
            const purchasesSpan = document.querySelector('.price-purchases');
            const combinedSpan = document.getElementById('combinedTotal');
            const remainingSpan = document.getElementById('remainingValue');

            // ✅ تحميل الحالة من localStorage
            document.addEventListener('DOMContentLoaded', function() {
                const savedMode = localStorage.getItem(bookingKey);
            ...
            toggleBtn?.addEventListener('click', function() {
                ...
                // 💾 حفظ بالحجز الحالي فقط
                localStorage.setItem(bookingKey, isMinutesMode);

                applyMode();
            });

            function applyMode() {
                document.getElementById('modeText').innerText = isMinutesMode ? 'ON' : 'OFF';
                toggleBtn.style.background = isMinutesMode ? '#28a745' : '#212529';

                let hoursValue, purchasesValue, total, remaining;

                purchasesValue = parseFloat(purchasesSpan.textContent.replace(/[^\d.-]/g, '')) || 0;

                if (isMinutesMode) {
                    // استخدام hours بالدقائق
                    hoursValue = parseFloat("{{ number_format($booking_hours_total_with_minute ?? 0, 2, '.', '') }}");
                } else {
                    hoursValue = parseFloat("{{ number_format($hours_total ?? 0, 2, '.', '') }}");
                }

                hoursSpan.innerText = hoursValue.toFixed(2) + ' جنيه';

                total = hoursValue + purchasesValue;
                combinedSpan.innerText = total.toFixed(2) + ' جنيه';

                const deposit = parseFloat("{{ number_format($deposit_paid ?? 0, 2, '.', '') }}") || 0;

                remaining = Math.max(total - deposit, 0);
                remainingSpan.innerText = remaining.toFixed(2) + ' جنيه';

                // 🔥 تحديث الحقول المخفية للفورم قبل الإرسال
                document.getElementById('hours_total').value = hoursValue.toFixed(2);
                document.getElementById('remaining').value = remaining.toFixed(2); // مهم جدًا
            }
        </script>
        ...
        </div>@include('bookings.modals.session_time_slot')

        <script>
            // 1. التعريفات الأساسية (Constants)
            const bookingConfig = {
                hallId: {{ $booking->hall_id }},
                estimateUrl: "{{ route('estimate-time-slot') }}",
                storeUrl: "{{ route('time-slots.store', $booking->id) }}",
                csrfToken: "{{ csrf_token() }}",
                lastSlotEndTime: "{{ \Carbon\Carbon::parse($lastSlotEndTime)->format('Y-m-d\TH:i') }}"
            };

            document.addEventListener('DOMContentLoaded', function() {
                // --- العناصر الأساسية ---
                const modalEl = document.getElementById('sessionTimeSlot');
                const durationRow = document.getElementById('durationRow');
                const newSlotFormContainer = document.getElementById('newSlotFormContainer');
                const btnToggleNewSlot = document.getElementById('btnToggleNewSlot');
                const btnCalculateSlot = document.getElementById('btnCalculateSlot');
                const btnSaveNewSlot = document.getElementById('btnSaveNewSlot');
                const formTitle = document.querySelector('#newSlotFormContainer h6');

                // --- حقول الإدخال ---
                const inputSlotId = document.getElementById('editingSlotId');
                const inputStart = document.getElementById('slotStartTime');
                const inputEnd = document.getElementById('slotEndTime');
                const inputAttendees = document.getElementById('slotAttendees');
                const displayPrice = document.getElementById('slotEstimatedPrice');
                const displayDuration = document.getElementById('slotEstimatedDuration');
                const resultBox = document.getElementById('slotEstimationResult');

                // 2. إدارة فتح المودال
                if (durationRow) {
                    durationRow.addEventListener('click', () => {
                        if (isMinutesMode) {
                            alert('❌ أنت حالياً تستخدم نظام الحساب بالدقائق، لا يمكن تقسيم الوقت.');
                            return;
                        }
                ...
                // 3. تبديل (Toggle) فورم الإضافة وتصفيره
                if (btnToggleNewSlot) {
                    btnToggleNewSlot.addEventListener('click', () => {
                        const isHidden = newSlotFormContainer.style.display === 'none';
                        if (isHidden || inputSlotId.value !== "") {
                            resetSlotForm();
                            newSlotFormContainer.style.display = 'block';
                        } else {
                            newSlotFormContainer.style.display = 'none';
                        }
                    });
                }
                ...
                // 4. حساب التكلفة (Ajax)
                if (btnCalculateSlot) {
                    btnCalculateSlot.addEventListener('click', async () => {
                        const start = new Date(inputStart.value);
                        const end = new Date(inputEnd.value);
                        const duration = Math.round((end - start) / 60000);

                        if (duration < 30) return alert('أقل مدة هي 30 دقيقة');

                        try {
                            const response = await fetch(bookingConfig.estimateUrl, {
                                method: 'POST',
                                headers: {
                                    'Content-Type': 'application/json',
                                    'X-CSRF-TOKEN': bookingConfig.csrfToken
                                },
                                body: JSON.stringify({
                                    hall_id: bookingConfig.hallId,
                                    attendees: inputAttendees.value,
                                    duration_minutes: duration
                                })
                            });
                            const data = await response.json();
                            if (data.success) {
                                displayPrice.innerText = data.estimated_formatted;
                                displayDuration.innerText = duration;
                                resultBox.style.display = 'block';
                            } else {
                                alert(data.error || 'خطأ في الحساب');
                            }
                        } catch (err) {
                            console.error(err);
                        }
                ...
                if (btnSaveNewSlot) {
                    btnSaveNewSlot.addEventListener('click', async () => {
                        const id = inputSlotId.value;
                        const isUpdate = id !== "";
                        const url = isUpdate ? `/time-slots/${id}` : bookingConfig.storeUrl;
                        ...
                        try {
                            const response = await fetch(url, {
                                ...
                                body: JSON.stringify({
                                    start_time: inputStart.value,
                                    end_time: inputEnd.value,
                                    attendees_count: inputAttendees.value,
                                    total_amount: displayPrice.innerText.replace(',', '')
                                })
                            });
                ...
                // 6. التعامل مع أزرار الجدول (تعديل وحذف)
                modalEl.addEventListener('click', function(e) {
                    // أ - التعديل
                    if (e.target.closest('.edit-slot-btn')) {
                        const btn = e.target.closest('.edit-slot-btn');
                        inputSlotId.value = btn.dataset.id;
                        inputStart.value = btn.dataset.start;
                        inputEnd.value = btn.dataset.end;
                        inputAttendees.value = btn.dataset.attendees;

                        formTitle.innerText = "تعديل الفترة المختارة";
                        newSlotFormContainer.style.display = 'block';
                        resultBox.style.display = 'none';
                        newSlotFormContainer.scrollIntoView({
                            behavior: 'smooth'
                        });
                    }
                    ...
                    if (e.target.closest('.delete-slot-btn')) {
                        const btn = e.target.closest('.delete-slot-btn');
                ...
                async function deleteSlot(id) {
                    try {
                        const response = await fetch(`/time-slots/${id}`, {
                            method: 'DELETE',
                            headers: {
                                'X-CSRF-TOKEN': bookingConfig.csrfToken
                            }
                ...
                // دالة مساعدة لتفريغ الفورم
                function resetSlotForm() {
                    inputSlotId.value = "";
                    formTitle.innerText = "حساب فترة جديدة";
                    inputStart.value = bookingConfig.lastSlotEndTime;
                    resultBox.style.display = 'none';
                }
            });
        </script>
