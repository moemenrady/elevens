...
class InvoiceController extends Controller
{
  ...
  public function ajaxSearch(Request $request)
  {
    ...
    $query = Invoice::query()
      ->with(['client', 'booking', 'items.session'])
      ->where('total', '>', 0);

    if ($q = $request->query('q')) {
      $query->where(function ($qq) use ($q) {
        $qq->whereHas('client', fn($q2) => $q2->where('name', 'like', "%{$q}%"))
    ...
    $invoices = $query->orderByDesc('updated_at')
      ->offset($offset)
      ->limit($limit)
      ->get();
    ...
    return response()->json([
      'data' => $invoices->map(function ($inv) {
        ...
        if ($inv->type === 'booking' && $inv->booking) {
          $start = $inv->booking->real_start_at;
          $end = $inv->booking->real_end_at;
        }

        if ($inv->type === 'session') {
          $sessionItem = $inv->items->firstWhere('session_id', '!=', null);
          if ($sessionItem && $sessionItem->session) {
            $start = $sessionItem->session->start_time;
            $end = $sessionItem->session->end_time;
          }
        }
  ...
  public function store(StoreInvoiceRequest $request, ShiftService $shiftService)
  {
    ...
    if ($invoice && !$isAdmin) {
      ShiftAction::create([
        ...
        'payment_type' => $request->payment_type, // 👈 هنا
        'amount' => $invoice->total,
        'expense_amount' => null,
